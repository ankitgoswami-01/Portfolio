...
document.addEventListener("DOMContentLoaded", function () {
  ...
  form.addEventListener("submit", function (event) {
    ...
    // Apna WhatsApp number (sirf digits me, bina '+' ke)
    const whatsappNumber = "918769745934";

    // Message text banana (encodeURIComponent special characters handle karta hai)
    ...
    // Link ko nayi tab me open karna
...
function updateTime() {
    // new Date() se current time le raha hai
    let now = new Date();

    // hours, minutes, AM/PM format
...
document.addEventListener("DOMContentLoaded", function() {
    const navLinks = document.querySelectorAll(".right-nav .scroll-to");
    const sections = Array.from(navLinks)
        .map(link => document.querySelector(link.getAttribute("href")))
        .filter(Boolean);

    const threshold = 200; // Delay before activating next section
    ...
    navLinks.forEach(link => {
        link.addEventListener("click", function(e) {
            ...
            // 🧠 Offset adjust — agar upar ja raha hai to 0 ya 20 try karo
            const offset = 20; // 👈 change this value if needed (e.g., 20 or 40 if you have a fixed header)
    ...
    window.addEventListener("scroll", () => {
        ...
        sections.forEach(section => {
            ...
            // delay so next section activates after some scroll
            if (window.scrollY >= sectionTop + threshold - sectionHeight / 2 && window.scrollY < sectionTop + sectionHeight) {
